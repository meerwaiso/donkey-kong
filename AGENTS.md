...
4. Document the design as a technical design doc in a Jira comment or sub-task.
5. Execution complete—do not load any code.
...
4. When reviewd task (task must be in review column and assigned to architect-agent) in jira, then move it to the next colomn (Testen) and assign it to tester-agent. Also review the pr in gitub.
...
7. Move the Jira ticket status to "In Review" and assign it to architect-agent.
8. Leave allways a comment in a task which you accompmlish. Start your comment with your role: developer-agent:....
...
5. Execute the tests.
6. Report results in a Jira comment and link the corresponding PR.
7. When test was successful then move the ticket to the next column(Deployment) and assign it to devops-agent. If not then move it back 2 colomns (Implementierung) and assign it to the developer-agent.
